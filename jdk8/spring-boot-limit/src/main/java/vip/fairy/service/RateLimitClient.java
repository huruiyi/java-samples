...
import vip.fairy.Constants;
import vip.fairy.enums.Token;
...
public class RateLimitClient {
  ...
  /**
   * 根据请求值去令牌桶获取，之所以用Long是因为lua脚本返回值用Integer接收有问题 lua要不直接返回json然后转map，为了省事直接返回list
   * <p>
   * list.get(0) 距离下一次刷新的最小时间间隔，单位：微秒 list.get(1) 获取到的令牌数
   *
   * @param key
   * @param permits
   * @return
