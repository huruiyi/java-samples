package vip.fairy.xml.appa.ch04.ParseXMLDoc;
    ...
    try (FileReader fr = new FileReader(args[0])) {
      ...
      while ((item = xmlsr.next()) !=
        ...
        switch (item) {
          ...
          case XMLStreamReader.START_ELEMENT:
            ...
            out.printf("Name = %s%n", xmlsr.getName());
            out.printf("Local name = %s%n", xmlsr.getLocalName());
            int nAttrs = xmlsr.getAttributeCount();
            for (int i = 0; i < nAttrs; i++) {
              out.printf("Attribute [%s,%s]%n", xmlsr.getAttributeLocalName(i), xmlsr.getAttributeValue(i));
            }
        }
      }
      ...
      if (xmlsr != null) {
        try {
          ...
          err.printf("XMLSE: %s%n", xmlse.getMessage());
        }
      }
    }
  }
