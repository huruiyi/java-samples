...
//以下程序段是用来计算输入日期为该年的第几天的
  ...
  public static int cptDay(int year, int month, int day) {
    byte dayAdd[] = {1, -2, 1, 0, 1, 0, 1, 1, 0, 1, 0, 1};
    int daycount = 0;
...
//以下程序段是来计算输入日期为该年第几周的
  ...
  public static int weekCount(int year, int month, int day) {
    ...
    int weekCnt = 0;
    if (dayCnt % 7 == 0) {
      weekCnt = dayCnt / 7 + ((weekminus > 0) ? 1 : 0);
    } else {
      weekCnt = dayCnt / 7 + ((weekminus > 0) ? 2 : 1);
    }
    return weekCnt;
  }
...
//打印万年历
  ...
  public static void main(String[] args) {
    String week[] = {"星期一", "星期二", "星期三", "星期四", "星期五", "星期六", "星期日"};
    System.out.println("输入的日期是该年的第" + cptDay(2019, 2, 15) + "天");
    System.out.println("这一天是该年的第" + weekCount(2019, 2, 15) + "周 " + week[getWeek(2019, 2, 15)]);
    printCal(2009);
  }
}
