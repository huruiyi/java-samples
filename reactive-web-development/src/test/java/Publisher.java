...
public class Publisher {
    ...
    void flowDelivery(final long sleepOrGoOutTimeclient1,
        ...
        final SubmissionPublisher<Integer> orderPublisher = new SubmissionPublisher<>(ForkJoinPool.commonPool(), maxItems);
        ...
        IntStream.rangeClosed(1, 100).forEach((number) -> {
            ...
            final int deliverLag = orderPublisher.offer(
                    ...
                    TimeUnit.SECONDS,
                    (customer, response) -> {
                        RuntimeException runtimeException = new RuntimeException("The " + ((Subscriber) customer).clientSubscriberName() + "unable to receive in the desired time" + response);
                        customer.onError(runtimeException);
                        return false;
                    });
            if (deliverLag < 0) {
                //failed items
                ...
                System.out.println("still left  " + deliverLag + " to be removed");
            }
        });
