...
public class Subscriber implements Flow.Subscriber<Integer> {
    ...
    public void onNext(final Integer itemNumber) {
        if (itemNumber != nextOrderIDItemExpected) {
            IntStream.range(nextOrderIDItemExpected, itemNumber)
                    .forEach((orderItemNumber) -> System.out.println("customer ended up leaving the house and missed the delivery " + orderItemNumber));
        ...
        System.out.println("I'll get another item now, next one should be: " + nextOrderIDItemExpected);
        clientSubscription.request(1);
    }
    ...
    public void onComplete() {
        System.out.println("I received all items in my order, thank you! Were" + totalItems + " in total");
    }
