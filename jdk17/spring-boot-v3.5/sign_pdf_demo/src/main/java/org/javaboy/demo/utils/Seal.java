...
//            FontMetrics fm = sun.font.FontDesignMetrics.getMetrics(f);
  ...
  private static void drawFont(Graphics2D g2d, int circleWidth, int circleHeight, SealFont font) {
    ...
    if (fontTexts.length > 1) {
      ...
      //5.设置上边距，默认在中心
      float margin = font.getMargin() == null ? (float) (circleHeight / 2 - rectangle2D.getCenterY())
...
//                ng2d.setBackground(Color.WHITE);
   ...
   * 画三字
   */
  private static BufferedImage drawThreeFont(BufferedImage bi, Graphics2D g2d, SealFont font, int lineSize, int imageSize, int fixH, int fixW,
      boolean isWithYin) {
    fixH -= 9;
