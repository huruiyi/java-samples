package com.example.demo.chapter_2.chapter_5;
...
public class ReactiveSalaryServiceIntegrationTest {
    ...
    public void shouldCreateAndRetrieveSalary() {
        ...
        SalaryDto body = response.getBody();
        System.out.println(body);

        assertThat(body).isNotNull();

        //when
        List<SalaryDto> getResponse = restTemplate.exchange(createTestUrl("/reactive/salary/" + body.getUserId()), HttpMethod.GET, null, new ParameterizedTypeReference<List<SalaryDto>>() {
        }).getBody();
