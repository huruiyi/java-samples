...
import com.example.demo.salary.infrastructure.security.SecurityConfigV2;
...
@ContextConfiguration(classes = {DemoApplication.class, SecurityConfigV2.class})
...
public class MVCControllerTest {
    ...
    public void shouldReturnAllsalaries() throws Exception {
        //given
        Iterable<Salary> salaries = Arrays.asList(new Salary("u1", "from1", "to1", 23L));
        when(salaryRepository.findAll()).thenReturn(salaries);

        //when, then
        this.mockMvc.perform(get("/mvc/all-salaries")).andDo(print()).andExpect(status().isOk()).andExpect(content().string(containsString("u1"))).andExpect(content().string(containsString("from1"))).andExpect(content().string(containsString("to1"))).andReturn();
    }

    @Test
    public void shouldReturnFormForCreatingSalary() throws Exception {
        //when, then
        this.mockMvc.perform(get("/mvc/createSalary")).andDo(print()).andExpect(status().isOk()).andExpect(content().string(containsString("<form action=\"/mvc/salary\" method=\"post\">\n" + "    <p>userId: <input type=\"text\" id=\"userId\" name=\"userId\" value=\"\"/></p>\n" + "    <p>accountFrom: <input type=\"text\" id=\"accountFrom\" name=\"accountFrom\" value=\"\"/></p>\n" + "    <p>accountTo: <input type=\"text\" id=\"accountTo\" name=\"accountTo\" value=\"\"/></p>\n" + "    <p>amount: <input type=\"number\" id=\"amount\" name=\"amount\" value=\"\"/></p>\n" + "    <p><input type=\"submit\" value=\"Submit\"/> <input type=\"reset\" value=\"Reset\"/></p>\n" + "</form>")))
    ...
    public void shouldPostNewSalary() throws Exception {
        ...
        mockMvc.perform(post("/mvc/salary").contentType(MediaType.APPLICATION_FORM_URLENCODED).sessionAttr("salaryDto", salaryDto)).andExpect(status().isOk()).andExpect(view().name("allsalaries"));
    }
}