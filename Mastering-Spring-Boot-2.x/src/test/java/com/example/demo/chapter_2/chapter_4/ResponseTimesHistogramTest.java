...
public class ResponseTimesHistogramTest {
    ...
    public void shouldMeasureActionsBuildingHistogram() {
        ...
        DistributionSummary distributionSummary = metricRegistry.summaryBuilder("request-simple.size").create();
    ...
    public void shouldMeasureActionsBuildingHistogramWithPercentiles() {
        ...
        DistributionSummary distributionSummary =
                metricRegistry.summaryBuilder("request-percentiles.size")
                        .quantiles(WindowSketchQuantiles.quantiles(0.3, 0.5, 0.99).create()).create();
        ...
        metricRegistry.findMeter(Meter.Type.DistributionSummary, "request-percentiles.size")
                .get().measure().iterator().forEachRemaining(measurements::add);

        List<String> result = extractPercentilesAsMap(measurements, "request-percentiles.size");
        assertThat(result).contains("0.3=20.0", "0.5=20.0", "0.99=100.0");
    ...
    public void shouldMeasureActionsBuildingHistogramWithLinear() {
        ...
        MeterRegistry metricRegistry = new PrometheusMeterRegistry();
        DistributionSummary distributionSummary =
                metricRegistry.summaryBuilder("request-summary.size")
                        .histogram(new NormalHistogram<>(NormalHistogram.linear(0, 10, 5))).create();
        ...
        assertThat(result).contains("10.0=2.0", "20.0=100.0", "+Inf=1.0");

    }

    private List<String> extractPercentilesAsMap(List<Measurement> measurements, String name) {
        return measurements.stream().filter(m -> m.getName().equals(name))
                .map(m -> m.getTags().first().getValue() + "=" + m.getValue())
