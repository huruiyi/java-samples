...
public class Tomcat {
  ...
  private static List<String> getStrings(String version) {
    ...
    urls.add(MessageFormat.format("https://archive.apache.org/dist/tomcat/tomcat-{1}/v{0}/bin/apache-tomcat-{0}-deployer.tar.gz", version,BIG_VERSION));
    ...
    urls.add(MessageFormat.format("https://archive.apache.org/dist/tomcat/tomcat-{1}/v{0}/bin/apache-tomcat-{0}-fulldocs.tar.gz", version,BIG_VERSION));
    urls.add(MessageFormat.format("https://archive.apache.org/dist/tomcat/tomcat-{1}/v{0}/bin/apache-tomcat-{0}-windows-x64.zip", version,BIG_VERSION));
    if (!BIG_VERSION.equals("11")) {
      urls.add(MessageFormat.format("https://archive.apache.org/dist/tomcat/tomcat-{1}/v{0}/bin/apache-tomcat-{0}-windows-x86.zip", version,BIG_VERSION));
    }
    ...
    urls.add(MessageFormat.format("https://archive.apache.org/dist/tomcat/tomcat-{1}/v{0}/bin/embed/apache-tomcat-{0}-embed.tar.gz", version,BIG_VERSION));
    urls.add(MessageFormat.format("https://archive.apache.org/dist/tomcat/tomcat-{1}/v{0}/bin/embed/apache-tomcat-{0}-embed.zip", version,BIG_VERSION));
