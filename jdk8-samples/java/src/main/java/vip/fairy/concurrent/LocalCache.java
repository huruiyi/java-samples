...
/**
 * 本地缓存 采用懒过期模式 在查询时才判断是否过期 在缓存满了的时候触发主动过期过期
 *
...
public class LocalCache {
  ...
  public final Map<String, Value> cache; //真正存储数据的Map，使用ConcurrentHashMap
  ...
  /**
   * 添加 判断是否超过最大限制 如果超过触发一次全量过期 如果全量过期后还不够返回false 由于1 2 不是原子的所以需要使用单独的AtomicInteger来控制
   *
   * @param key    对应的key
   * @param value  值
  ...
  /**
   * 为了保证cur和Map的size时刻保持一致这里我查询了put的注释及ConcurrentHashMap底层关于put的实现。 发现如果put方法返回的不是null说明存在覆盖操作，如果是覆盖那么Map的size其实没有变，因为我们添加之前把cur的值增加 上去了所以要在这里减下来。
   */
  ...
  /**
   * 这里也是为了保证cur和Map的size时刻保持一致只有在remove方法返回的不是null时才证明真正有对象被删除了，才需要把 cur减下来。这里出现remove返回为null是因为可能存在并发删除，两个线程删除同一个对象只能有一个删除成功(返回不是
  ...
  /**
   * 这里很重要，原来我使用的是cache.size() >= maxNumber; 但是如果使用map本身的size方法会存在获取size和putValue方法不是原子的，
